...
### Added
- `Bucket#stats` returns a frozen snapshot of `{ available, capacity, refill_rate, strategy }`.
- `Bucket#take_wait_timeout(n, timeout:)` blocks up to `timeout` seconds waiting for tokens; raises on timeout.
- YARD documentation on all public instance methods.

## [0.3.0] - 2026-04-10

### Added
- `#reset` method to restore tokens to full capacity and reset the refill timer
- `#refill_rate` reader to expose the configured refill rate
- `#strategy` reader to expose the configured refill strategy

### Changed
- README Usage section restructured to follow template guide
